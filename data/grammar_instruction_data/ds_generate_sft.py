...
def main_fn(df_dict,prompt):
    ...
    outjsonl = open(f'2.gpt对目标语法点生成sft数据/deepseek_教学指令数据生成0626.jsonl', 'a+', encoding='utf-8')

    for level,df in df_dict.items():
        loop = tqdm(range(0,len(df)),desc=f'Processing {level}... [Text ID: X] [Valid Num: {valid_count}]',colour='blue')
        instruction_record = []
        for i in loop:
            if level == '3级':
                continue
            ...
            for pool_num in tqdm(range(instruction_num)):
                now_prompt = prompt.replace('[Instruction Pool]',str(instruction_record[-3:])).replace('[New Grammar Point]',input_str)
    ...
    result_df.to_excel(f'2.gpt对目标语法点生成sft数据/deepseek_教学指令数据生成0626.xlsx', index=False)

if __name__ == '__main__':
    df_dict = pd.read_excel('1.目标语法点整理/目标语法项目_拆分.xlsx',sheet_name=None)
    prompt = open('2.gpt对目标语法点生成sft数据/prompts/1.对语法项目生成sft数据.txt', 'r', encoding='utf-8').read()
